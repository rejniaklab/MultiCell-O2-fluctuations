function script_uptake
  ...
  for num_exp=1:4      % select the tissue for each ROI
    ...
    % determine the optimal schedule for uptake for each segment
     Main_fluct_optim_uptake(Vfrac,Tfrac,Sfrac,textExp,pathOutTiss)
     
    
    
    % run the determined optimal schedule for all segments 
    [r2,norm2]=oxy_tissue_fluct_uptake(Vfrac,Tfrac,Sfrac,'tissue',...
               pathOutTiss,num_exp);
    ...
    % final data to save  
      ...
      save([pathOutTiss,'/final_',textExp,'_uptake.txt'],'save_data','-ascii')
