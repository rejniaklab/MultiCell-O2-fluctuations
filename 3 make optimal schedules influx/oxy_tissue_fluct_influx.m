function [r2,norm_res]=oxy_tissue_fluct_influx(Vfrac,Tfrac,Sfrac,...
...
% exp_num -- determines expeirmental data to read
  ...
  influx=load([tiss_pathOut,'/rateInflux_opt_influx.txt']);
  uptake=load([tiss_pathOut,'/rateUptake_opt_influx.txt']);
  ...
  pathData=[tiss_pathOut,'/DataFinalInfux']; % name of the directiory for saving data, 2 for influx, 3 for uptake
  pathFigs=[tiss_pathOut,'/FigsFinalInfux']; % name of the directiory for saving figs    
  ...
  oxy    =load([tiss_pathOut,'/Data_influx/oxy_1.txt']); %%KAR 11/25
  Nvess=size(xyVess,1);
  Ntum=size(xyTum,1);
  ...
  if (mod(iter,Ndraw)==0)
    ...
    if (iter==Niter)
      ...
      title(['Average O2: ',num2str(mean(mean(oxy))),';    R2= ',...
          num2str(r2),';    norm= ',num2str(norm_res),' case: influx'])
    end
...
if (toSavelast==1)   
  ...
  save([tiss_pathOut,'/parametersFluct_influx.txt'],'parameters','-ascii')
