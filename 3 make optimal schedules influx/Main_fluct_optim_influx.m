function Main_fluct_optim_influx(Vfrac,Tfrac,Sfrac,textExp,pathOutTiss)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Optimizing influx schedule to match experimental oxygen fluctuation %
% data for textExp:                                                   %
...
global pathOutTiss pathOutSch Vfrac Tfrac Sfrac textExp 
  ...
  pathOutSch=[pathOutTiss,'/Data_influx']  
  if ~exist(pathOutSch, 'dir') mkdir(pathOutSch); end  

  
  % initial guess and lower & upper limits
  x0 =[1];    % initial guess for influx
  lb =[0];    % lower bound for influx
  ub =[1];    % upper bound for influx
  ...
  rate_influx_tosave=zeros(9,1);
  rate_uptake_tosave=ones(9,1);
  ...
  for time=1:9    
    ...
    rate_influx_tosave(time,1)=rate_opt; 
      oxy_level_tosave(time,1)=exp_oxy(time,2)-oxy_diff; 
   
    % save the whole data vector for the current results 
    save([pathOutTiss,'/rateInflux_opt_influx.txt'],'rate_influx_tosave','-ascii')
    save([pathOutTiss,'/rateUptake_opt_influx.txt'],'rate_uptake_tosave','-ascii')
    save([pathOutTiss,'/oxyLevel_opt_influx.txt'],'oxy_level_tosave','-ascii')
...
global pathOutSch 
  ...
  disp(['time: ',num2str(time),'  iteration: ',num2str(iter_opt),...
        '  influx: ',num2str(rate_in)]);
  
  % run our oxygenation model to return average level of oxygen 'oxy_out'
  oxy_out = run_fluct_optim_influx(rate_in,time,Vfrac,Tfrac,Sfrac,exp_oxy);
  ...
  % drawing and saving results
    ...
    subplot(2,1,1)
      ...
      axis([0,iter_opt+1,0,1])
      hold on  
      grid on
      xlabel('iteration')
      ylabel('influx rate')
     
      title(['Influx rate=',num2str(rate_in),';  optim=',num2str(oxy_out),...
   ...
   figure(10)
   print('-djpeg',[pathOutSch,'/fig_influx_',num2str(time),'.jpg'])
